const toReadable = (x) =>{
    ...
    const arr2 = ["","","twenty", "thitry", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"];
    ...
    else if (x > 999 && x <= 19999){let thous = Math.floor(x / 1000);
    ...
    if(hunds === 0)            {
    console.log(`${arr[thous]} ${thousand} ${arr2[tens]} ${arr[units]}`);
    return (`${arr[thous]} ${thousand} ${arr2[tens]} ${arr[units]}`);
                             }
    else                     {
    console.log(`${arr[thous]} ${thousand} ${arr[hunds]} ${hundred} ${arr2[tens]} ${arr[units]}`);
    return (`${arr[thous]} ${thousand} ${arr[hunds]} ${hundred} ${arr2[tens]} ${arr[units]}`);
                             }
                             }
    
    if (x > 99 && x < 1000) {let hunds = Math.floor(x / 100);
    ...
    else if (x >= 20 && x < 100) {let tens = Math.floor(x / 10);
                             let units = x - (tens * 10);
