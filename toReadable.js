const toReadable = (x) =>{
    ...
    else  if (x > 99 && x < 1000) {let hunds = Math.floor(x / 100);
                             let tens = Math.floor((x - (hunds * 100)) / 10);
                             let units = x - ((hunds * 100) + (tens * 10));
                             
            if (tens < 2 ){
                             console.log((`${arr[hunds]}${hundred}${arr[x - (hunds * 100)]}`).trim());
                             return ((`${arr[hunds]}${hundred}${arr[x - (hunds * 100)]}`).trim());}
            else if (tens >= 2){
                             console.log((`${arr[hunds]}${hundred}${arr2[tens]}${arr[units]}`).trim());
